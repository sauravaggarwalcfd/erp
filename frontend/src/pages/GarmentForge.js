...
const GarmentForge = () => {
  ...
  const fetchAllData = async () => {
    try {
      const [statsRes, tasksRes, ordersRes, notificationsRes, materialsRes] = await Promise.all([
        axios.get(`${API}/analytics/dashboard`),
        axios.get(`${API}/tasks`),
        axios.get(`${API}/orders`),
        axios.get(`${API}/notifications?user_id=${currentUser.id}`),
        axios.get(`${API}/materials?low_stock=true`)
      ]);
      ...
      setLowStockMaterials(materialsRes.data.slice(0, 5));
      setLoading(false);
    } catch (error) {
  ...
  const moduleCards = [
    ...
    },
    {
      ...
      features: [\"Interactive Notifications\", \"Group Chat Integration\", \"File Attachments (6MB)\", \"Recurring Tasks\", \"Time Tracking\"]
    },
    {
    ...
    },
    {
      ...
      features: [\"Skills Tracking\", \"Department Organization\", \"Performance Monitoring\", \"Attendance\"]
    },
    {
              ...
              <div className=\"bg-white rounded-2xl p-6 shadow-lg\">
                ...
                <div className=\"space-y-3 max-h-64 overflow-y-auto\">
                  {notifications.map((notif, idx) => (
                    <div key={idx} className=\"bg-gray-50 p-3 rounded-lg hover:bg-gray-100 transition cursor-pointer\"
                      ...
                      <div className=\"flex items-center gap-2 mb-1\">
                        ...
                        <p className=\"font-medium text-sm text-gray-800 truncate\">{notif.title}</p>
                        {!notif.read && <span className=\"w-2 h-2 bg-blue-500 rounded-full\"></span>}
                      </div>
                      <p className=\"text-xs text-gray-600\">{new Date(notif.created_at).toLocaleDateString()}</p>
                    </div>
                  ))}
